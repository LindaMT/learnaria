;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype.togglePanel = function (event) {
		...
		var plugin, $elem, $panel, $me, isVisible, $currentButton;
		...
		if(plugin.options.autoCollapse) { // expand current panel and collapse the rest
			...
			plugin.headers.each(function(i, el) {
				...
				if($btn[0] != $(event.currentTarget)[0]) { 
					// Identify button as collapsed
					$btn.attr({'aria-expanded': 'false'}).removeClass('expanded');
					// Identify panel controlled by this button as hidden; aria-controls contains its name
					$currentPanel = $("#"+$btn.attr('aria-controls'));
					$currentPanel.attr( {'aria-hidden': 'true'});
					$hdr.next().slideUp(plugin.options.animationSpeed);
				} else { 
					// Identify button as expanded
					$btn.attr({'aria-expanded': 'true'}).addClass('expanded');
					// Identify panel controlled by this button as visible
			...
			$currentButton = $panel.prev().find(".button");
